...
Lenbs <- new("LB_lengths", 
             LB_pars=MyPars, 
             file=paste0(datdir, "/data/lenghtBS_nochina.csv"), 
             dataType="freq",
             sep=",",
             header=T)
...
Lenei <- new("LB_lengths", 
             LB_pars=MyPars, 
             file=paste0(datdir, "/data/lenghtEI_nochina.csv"),
             dataType="freq",
             sep=",",
             header=T)
#Gerlache Strata
Lengc <- new("LB_lengths", 
             LB_pars=MyPars, 
             file=paste0(datdir, "/data/lenghtExtra_nochina.csv"), 
             dataType="freq",
             sep=",",
             header=T)
#Join Strata
Lenjo <- new("LB_lengths", 
             LB_pars=MyPars, 
             file=paste0(datdir, "/data/lenghtJOIN_nochina.csv"), 
             dataType="freq",
             sep=",",
             header=T)
#SSIW Strata
Lenssiw <- new("LB_lengths", 
               LB_pars=MyPars, 
               file=paste0(datdir, "/data/lenghtSSIW_nochina.csv"), 
               dataType="freq",
               sep=",",
               header=T)
```
...
```{r Table5, message=FALSE, echo=FALSE}
tablk <- valprotodo %>% 
  group_by(Parameter, SPRstra) %>% 
  summarise(Median = mean(SPR),
            Variance=var(SPR),
...
Len481 <- new("LB_lengths", 
             LB_pars=MyPars, 
             file=paste0(datdir, "/data/lenght481_nochina.csv"), 
             dataType="freq",
             sep=",",
             header=T)
...
Len482 <- new("LB_lengths", 
             LB_pars=MyPars, 
             file=paste0(datdir, "/data/lenght482_nochina.csv"),
             dataType="freq",
             sep=",",
             header=T)
...
Len483 <- new("LB_lengths", 
             LB_pars=MyPars, 
             file=paste0(datdir, "/data/lenght483_nochina.csv"), 
             dataType="freq",
             sep=",",
             header=T)
```
...
glossary <- tribble(
  ...
  # Biological & life-history terms
  "Intrinsic Productivity", "Capacity of a population to reproduce and sustain itself, which depends on reproductive and somatic conditions.",
  "SPR", "Spawning Potential Ratio. Ratio of egg production per recruit under fishing to unfished conditions; key BRP.",
  "LBSPR", "Length-Based Spawning Potential Ratio model used to estimate stock status from length data.",
  "VBGF", "Von Bertalanffy Growth Function. Growth model describing length-at-age through parameters Linf, k, and t0.",
  "Linf", "Asymptotic length; theoretical maximum body size.",
  "k", "Growth coefficient; rate at which individuals approach Linf.",
  "M/k", "Life-history invariant representing growth–mortality balance and productivity.",
  "M", "Natural Mortality. Instantaneous mortality in the absence of fishing.",
  "F/M", "Relative measure of fishing mortality compared to natural mortality.",
  "Maturity Ogive", "Logistic function describing proportion of mature individuals by length.",
  "L50", "Length at which half the population is mature.",
  "L95", "Length where 95% of individuals are mature.",
  "CVL", "Coefficient of variation in length used in LBSPR.",
  "Recruitment", "Entry of new individuals into the population or fishery.",
  "Recruitment Variability", "Temporal fluctuation in the number of recruits influencing productivity.",
  "EPR", "Egg Production per Recruit. Lifetime reproductive output of a recruit, fished or unfished.",
  "Growth Proxy", "Use of length metrics as indicators of physiological conditions.",


  # Fishery, selectivity & stock assessment
  "Length Composition", "Frequency distribution of observed total lengths from fishery-dependent data.",
  "Selectivity", "Probability that an individual of a given size enters the fishery.",
  "SL50", "Lengths at 50% and 95% probability of gear selectivity.",
  "SL95", "Length at 95% probability of gear selectivity.",
  "Fishery-Dependent Data", "Data collected onboard vessels such as SISO length compositions.",
  "Steady-State Distribution", "Expected equilibrium length distribution under constant F and M.",
  "BRP", "Biological Reference Point. Benchmark to evaluate stock status (e.g., SPR20%, SPR75%).",
  "LRP", "Limit Reference Point. Threshold below which the stock status is considered impaired (SPR20%).",
  "TRP", "Target Reference Point. Desired stock status objective (SPR75% for krill).",
  "MSY", "Maximum Sustainable Yield. Largest long-term average catch that can be sustainably taken.",
  "Hockey-Stick HCR", "Piecewise-linear harvest control rule used in fisheries management.",
  "Pseudo-Cohort Analysis", "Length-based analytical approach using multiple length frequencies (LBPA).",
  "Data-Poor Assessment", "Stock assessment using limited inputs",
  "Integrated Assessment Model", "Model synthesizing multiple data types within a unified population framework.",
  "MP", "Management Procedure. Pre-agreed algorithm determining harvest decisions under uncertainty.",
  "HCR", "Harvest Control Rule. Algorithm linking observed indicators (e.g., SPR) to allowed catch.",
  "Catch Limit", "The total combined catch of Euphausia superba in Statistical Subareas 48.1, 48.2, 48.3 and 48.4 shall be limited to 5.61 million tonnes in any fishing season (CM 51-07).",
  "Trigger Level", "Total combined catch in Statistical Subareas 48.1, 48.2, 48.3 and 48.4 shall be further limited to 620 000 tonnes in any fishing season (CM 51-07).",

  # CCAMLR & spatial management
  "CCAMLR", "Commission for the Conservation of Antarctic Marine Living Resources.",
  "SISO", "CCAMLR’s Scheme of International Scientific Observation providing fishery monitoring data.",
  "Subarea 48.1", "Main krill fishing area along the Western Antarctic Peninsula.",
  "Management Strata", "Spatial fishery management units in 48.1 (BS, EI, GS, JOIN, SSWI).",
  "SSMU", "Small-Scale Management Unit used for ecosystem-based krill management.",
  "CM 51-07", "Conservation Measure specifying spatial distribution of the trigger level; expired in 2024.",
  "Spatial Heterogeneity", "Differences in productivity or population structure across regions.",

  # Oceanographic & environmental variables
  "WAP", "Western Antarctic Peninsula, a region experiencing rapid warming and sea-ice decline.",
  "SST", "Sea Surface Temperature; key environmental driver affecting krill growth and productivity.",
  "SIC", "Sea Ice Concentration derived from Nimbus-7; influences recruitment.",
  "Chlorophyll-a", "Proxy for primary productivity and food availability.",
  "Environmental Covariate", "External variable influencing krill growth dynamics.",

  # Modeling, statistics & computation
  "GLMM", "Mixed-effects model incorporating fixed effects and random structure.",
  "Pearson Correlation", "Correlation metric assessing relationship between length and environmental variables.",
  "Rasterization", "Conversion of spatial satellite data to uniform grids.",
  "sf", "R package for handling spatial vector data.",
  "ncdf4", "R interface for working with NetCDF satellite and oceanographic data."
)

```

```{r kable-version, results='asis', echo=FALSE}
glossary %>%
  kbl(
    booktabs = TRUE,
