...
*Keywords: Krill populations, Dynamic population, Size compositions, Spawning Potential Ratio (SPR), Management*
...
## 2.4. Assessment of intrinsic productivity
...
## 2.7. Sensitivity and perfomance analysis.
...
```{r fig.cap="\\label{Figure4}Fit of the model to the data of lengths in Braishfield strata", fig.height=5}
plotSize(fitei,
...
```{r fig.cap="\\label{Figure5}Fit of the model to the data of lengths in Joinville strata", fig.height=3, fig.width=4}
plotSize(fitjo,
...
```{r, fig.cap="\\label{Figure6}Difference between the observed accumulated size structure for each stratum related SPR objective", fig.height=5, fig.width=5}
...
```{r curve_mat, echo=FALSE, fig.cap= "\\label{Figure7}Maturity curves by strata", fig.height=7, fig.width=5}
...
## 3.2. Comparing producivity between years and Stratas
...
```{r warning=FALSE, fig.cap="\\label{Figure8}Krill Intrinsic Productivity (SPR) by strata and by year", fig.height=3, fig.width=7}
...
## 3.3. Sensitivity and perfomance analysis
...
```{r warning=FALSE, fig.cap="\\label{Figure9}lSensitivity analysis by strata about asymptotic length VB", fig.height=3, fig.width=8}
# plot all Linf
...
```{r warning=FALSE, fig.cap="\\label{Figure10}Sensitivity analysis by strata about krill growth type",  out.width='60%'}
#plot
# todo
...
kbl(tablk, 
    longtable = F, 
    booktabs = T, 
    caption = "\\label{Table4}Estimated by groth type scenario") %>% 
  add_header_above(c(" ", "Mean" = 5, "Variance" = 5)) %>% 
  kable_styling(latex_options = c("scale_down",  "hold_position"))
```
...
an individual with a lower growth ($L_{\infty}$ 55) has by definition to have a faster growth, and that is why the reproductive potential increases. This was possible to observe through the sensitivity analysis for this parameter in all strata.

Regarding variability in growth krill. 
Overall, our study found that life history as expressed
in the M/K ratio was a good predictor of the performance
of length-based mortality estimators, with better
performance in high-M/K scenarios compared to low-M/K
scenarios. This result supports those in previous studies of
length-based methods (e.g., Hordyk et al. 2015b). Two
features unique to low-M/K populations may result in
## 4.6. Anoher studies about Intrinsic productivity in krill
