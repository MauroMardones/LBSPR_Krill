...
```{r Figure 3, fig.height=5, fig.width=6, fig.cap="\\label{Figure3}Theoretical growth scenarios to krill varability"}
...
```{r warning=FALSE, fig.cap="\\label{Figure8}Krill Intrinsic Productivity (SPR) by strata and by year"}
...
write.csv(allsprwide, "SPRall.csv", sep="")
...
tablinf <- valtodo %>% 
  ...
  pivot_wider(names_from= SPRstra, 
              ...
              names_sep = " ") %>% 
  mutate_all(~na_if(., is.na(.)) %>% ifelse(is.na(.), "-", .)) %>% 
